...
export class UserPreferencesService {
  ...
  async findOne(id: number): Promise<UserPreference> {
    const userPreference = await this.userPreferenceRepository.findOne({ where: { id }, relations: ['user'] });
    if (!userPreference) throw new NotFoundException('User preference not found');
    return userPreference;
  }

  async update(id: number, updateUserPreferenceDto: UpdateUserPreferenceDto): Promise<UserPreference> {
  ...
  async remove(id: number): Promise<void> {
    ...
    if (result.affected === 0) throw new NotFoundException('User preference not found');
  }
}
