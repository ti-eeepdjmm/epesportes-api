...
describe('UsersController', () => {
  ...
  describe('findOne', () => {
    ...
    it('should throw NotFoundException if user is not found', async () => {
      jest.spyOn(service, 'findOne').mockRejectedValue(new NotFoundException(`User with ID 1 not found`));
  ...
  describe('update', () => {
    ...
    it('should throw an error if the user is not found', async () => {
      ...
      jest.spyOn(service, 'update').mockRejectedValue(new Error('User not found')); // Mocka um erro
  
      await expect(controller.update('1', updateUserDto)).rejects.toThrow('User not found'); // Verifica se o erro é lançado
